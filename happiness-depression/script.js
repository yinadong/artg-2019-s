...
console.log(DepressionbyGenderPromise);
...
Promise.all([
   ...
   const Depression_tmp = d3.nest()
	...
	//const Depressiontest = Array.from(DepressionBy2008.values());

	const DepressionbyGDPAugmented = DepressionBy2008.map(d => {
		console.log(d);

		//const happinessvalue = happinessMap.get(d.Countrycode);
		//console.log(happiness);

		// console.log(happiness);
		// const happinessvalue = happinessMap.get(d.Countrycode).value;
		// // console.log(happinessvalue);
		// const happinessyear = happinessMap.get(d.Countrycode).Year; //year d.Year can't match with happiness.Year
		//console.log(happinessyear);
		if(happiness) {
			const yearData = happiness.filter(function(dd) {
				return dd.key == year;

			})
			
			dd.year = yearData.key;
			dd.value = yearData.values.value
			console.log(dd.value);
            }
		// if(happinessyear){
		// 		d.year = happinessyear.Year;
		// 	}
		// if(happinessvalue){
		// 		d.happiness = happinessvalue.value; // get the d.origin_subregion
		//})
    ...
    /* const Depression = d3.nest()
      .key(d => d.Year)
      .key(d => d.Countryname)
      .entries(DepressionbyGDPAugmented);
     
     console.log(Depression) ; 

     const DepressionBy1990 = Depression
			.filter(function(d){return d.key === "1990"})[0]
			.values;

	console.log(DepressionBy1990);*/

	drawScatter(DepressionBy1990, plot);
});
...
function drawScatter(data, selection){
	...
	//Data discovery
	console.log(data);//have to convert to flat data
	
	const happinessExtent = d3.extent(data, function(d){ return d.value.happiness});
